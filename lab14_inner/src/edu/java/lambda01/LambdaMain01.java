...
public class LambdaMain01 {
    ...
    public static void main(String[] args) {
        ...
        // Calculable subtracter = new Subtracter(); <- 다형성
        result = subtracter.calculate(1, 2);
        System.out.println(result);
        
        // 3. 익명 클래스(anonymous class)
        Calculable multilpier = new Calculable() {
            @Override
            public double calculate(double x, double y) {
                return x * y;
            }
        };
        result = multilpier.calculate(1, 2);
        System.out.println(result);
...
//        Printable printer2 = (msg) -> System.out.println(msg.toUpperCase()); // body에 있는 {} 생략
        Printable printer2 = msg -> System.out.println(msg.toUpperCase()); // param에 있는 () 생략
        printer2.print("Hello, Lambda!");
                
        // 메서드 참조(method reference) 1:
        // 람다 표현식에서 파라미터가 1개이고, 메서드 body의 실행 문장도 1개 뿐인데,
        // 람다 표현식의 파라미터가 실행문장의 argument로 그대로 전달되는 경우.
//        Printable printer3 = msg -> System.out.println(msg);
        Printable printer3 = System.out::print;
